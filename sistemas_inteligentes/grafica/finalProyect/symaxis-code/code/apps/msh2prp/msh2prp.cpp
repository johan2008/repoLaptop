...
static int number_samples = 1000;
...
CreateVertexSampling(R3Mesh *mesh, int num_vertices, RNScalar *weights)
{
    ...
    start_time.Read();
    if (print_verbose) {
        printf("Create vertex sampling (%d->%d) ...\n", mesh->NVertices(), num_vertices);
        fflush(stdout);
    }
    ...
    if (mesh->NVertices() <= num_vertices) {
        ...
        for (int i = 0; i < num_vertices; i++) {
            ...
            delete [] distances;
			// Print progress bar
			PrintProgressBar(i*100/num_vertices);
        }
		printf("\n");
        ...
        if (weights) {
            for (int i = 0; i < selected_vertices->NEntries(); i++) {
                weights[i] = mesh->Area() / selected_vertices->NEntries();
            }
        }
    }
    if (print_verbose) {
...
ComputeCurvatureProperties(R3Mesh *mesh)
{
    ...
    // Allocate properites
    R3MeshProperty *gauss1 = new R3MeshProperty(mesh, "OldGaussCurvature");
    R3MeshProperty *gauss = new R3MeshProperty(mesh, "GaussCurvature");
    R3MeshProperty *mean1 = new R3MeshProperty(mesh, "OldMeanCurvature");
    ...
    for (int i = 0; i < nv; i++) {
        ...
        diagonalize_curv(pdir1[i], pdir2[i],
                         ...
                         curv1[i], curv2[i]);
        gauss1->SetVertexValue(i, mesh->VertexGaussCurvature(vertex));
        mean1->SetVertexValue(i, mesh->VertexMeanCurvature(vertex));
    ...
    // Insert properties at multiple scales
    InsertProperty(properties, gauss1);
    InsertProperty(properties, gauss);
    InsertProperty(properties, mean1);
...
static R3MeshPropertySet *
ComputeCurvaturePropertiesOld(R3Mesh *mesh)
{
    // Start statistics
    RNTime start_time;
    start_time.Read();
    ...
    for (int i = 0; i < mesh->NVertices(); i++) {
        R3MeshVertex *vertex = mesh->Vertex(i);
        gauss->SetVertexValue(i, mesh->VertexGaussCurvature(vertex));
        mean->SetVertexValue(i, mesh->VertexMeanCurvature(vertex));
    }
    
    // Blur curvatures a bit for stability
    gauss->Blur(Sigma(mesh));
    mean->Blur(Sigma(mesh));
...
/*    int nsamples_in_smallest_bin = 32;*/
    ...
    for (int i = 0; i < samples->NEntries(); i++) {
        ...
        // Print progress bar
        PrintProgressBar(i*100/samples->NEntries());
    }
	printf("\n");
...
{
    // Remember how many property sets user selected
    int nselected = 0;
    ...
    while (argc > 0) {
        if ((*argv)[0] == '-') {
            ...
            else if (!strcmp(*argv, "-basic")) { nselected++; compute_basic_properties = 1; }
            else if (!strcmp(*argv, "-curvature")) { nselected++; compute_curvature_properties = 1; }
            else if (!strcmp(*argv, "-volume")) { nselected++; compute_volume_properties = 1; }
            else if (!strcmp(*argv, "-laplacian")) { nselected++; compute_laplacian_properties = 1; }
            else if (!strcmp(*argv, "-dijkstra")) { nselected++; compute_dijkstra_distance_properties = 1; }
            else if (!strcmp(*argv, "-dijkstra_statistics")) { nselected++; compute_dijkstra_distance_properties = 1; }
            else if (!strcmp(*argv, "-dijkstra_histogram")) { nselected++; compute_dijkstra_histogram_properties = 1; }
            else if (!strcmp(*argv, "-num_samples"))
            {
                argv++;
                argc--;
                number_samples = atoi(*argv);
            }
            else if (!strcmp(*argv, "-raytrace")) { nselected++; compute_raytrace_properties = 1; }
            else if (!strcmp(*argv, "-solidtexture")) { nselected++; compute_solidtexture_properties = 1; }
            else if (!strcmp(*argv, "-map")) { nselected++; argv++; argc--; map_name = *argv; }
    ...
    if (!input_mesh_name || !output_properties_name) {
        fprintf(stderr, "Usage: msh2properties meshfile propertiesfile [options]\n");
        return 0;
    }
    
    // Select all properties if none explicitly selected
    if (nselected == 0) {
