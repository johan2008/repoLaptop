...
static int pca_scale = 1; // 2 = anisotropic
...
Align(R3Mesh *mesh1, int translation, int rotation, int scale)
{
  ...
  if (pca_scale == 2) {
    // Compute set of points
    const int max_points = 1024;
    R3Point *points = new R3Point [ max_points ];
    int npoints = CreatePoints(mesh1, points, max_points);
    if (npoints == 0) return 0;

    // THIS ONLY WORKS IF ROTATE FIRST

    // Compute covariance matrix
    R4Matrix m = R4null_matrix;
    R3Point centroid = mesh1->Centroid();
    for (int i = 0; i < npoints; i++) {
    ...
    // Compute square root of inverse of matrix
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < 3; i++) {
        if (m[i][j] != 0) m[i][j] = 1.0 / sqrt(fabs(m[i][j]));
      }
    }

    // Scale mesh anisotropically
    R3Affine anisotropic_affine(R3identity_affine);
    anisotropic_affine.Translate(centroid.Vector());
    anisotropic_affine.Transform(m);
    anisotropic_affine.Translate(-centroid.Vector());
    mesh1->Transform(anisotropic_affine);
  }
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      ...
      else if (!strcmp(*argv, "-anisotropic_scale")) { pca_scale = 2; icp_scale = 2; }
