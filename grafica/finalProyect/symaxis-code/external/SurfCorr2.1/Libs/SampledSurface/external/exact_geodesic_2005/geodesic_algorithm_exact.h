															   ...
															   IntervalWithStop* one)			//intersecting two intervals with up to three intervals in the end
{
	...
	{
		double x = inter[(int)i];
		if(x > left + local_epsilon && x < right - local_epsilon)
		{
			good_start[(int)(Ngood_start++)] = x;
		}
	}
	good_start[(int)(Ngood_start++)] = right;

	MapType mid_map[3];
	for(char i=0; i<Ngood_start-1; ++i)
	{
		double mid = (good_start[(int)i] + good_start[(int)(i+1)])*0.5;
		mid_map[(int)i] = zero->signal(mid) <= one->signal(mid) ? OLD : NEW;
	}
...
//		list_pointer list = interval_list(edge);	//vk: unused
