...
vec3 init_eye(9.0, 3.0, -1.0);
vec3 eye = init_eye;
vec3 _cameraDir( -9.0f, -3.0f, -1.0f );
...
void position_cero_level(model* g_model, vector<curveStruct> &model_curve ){
    ...
    for (int i = 0; i < 10; )
    {
        ...
        for(int n=0; n< positions.size(); n++)
        {
            ...
            color_element.y = 255;
            color_element.z = 0;
            _colors_cero_level.push_back(color_element);
        }
...
void init(){
    g_model  = new model( "../Data/SCAPE/Meshes/mesh009.off" , 0, 1);
    g_model2 = new model( "../Data/SCAPE/Meshes/mesh010.off" , 1, 1); 

    //g_model  = new model( "cat0.off" , 0);
    //g_model  = new model( "../Data/TOSCA/Meshes/cat0.off" , 0,150);
    //g_model2 = new model( "../Data/TOSCA/Meshes/cat0.off" , 1,150); 
...
void display(){
    ...
    //drawCeroLevel(cero_level_buffer, positions.size());  // draw the floor
