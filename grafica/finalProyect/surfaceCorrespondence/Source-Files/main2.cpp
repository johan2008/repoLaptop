...
GLuint line_buffer2;
GLuint line_buffer3;
GLuint line_buffer1_mesh2;
GLuint line_buffer2_mesh2;
GLuint line_buffer3_mesh2;
GLuint cero_level_buffer1[30];
GLuint cero_level_buffer2[30];
GLuint EBO;
GLuint EBO2;
...
vec3 init_eye(9.0, 3.0, -2.0);
vec3 eye = init_eye;
vec3 _cameraDir( -9.0f, -3.0f, 2.0f );
...
point3 Line1_mesh2[2] = {
    point3( 0.0, 0.0,  0.0),
    point3( 00.0, 0.0, 3.0)
};
color3 vertex_colors_Line1_mesh2[2] = {
    color3( 0.0, 0.0, 1.0),  // 
    color3( 0.0, 0.0, 1.0)  // 
};

//RED AXI-X
point3 Line2_mesh2[2] = {
    point3( 0.0, 0.0,  0.0),
    point3( 3.0, 0.0, 00.0)
};
color3 vertex_colors_Line2_mesh2[2] = {
    color3( 1.0, 0.0, 0.0),  // 
    color3( 1.0, 0.0, 0.0)  // 
};

//GREEN AXI-Y
point3 Line3_mesh2[2] = {
    point3( 0.0 , 0.0,  0.0),
    point3( 00.0,3.0, 0.0)
};
color3 vertex_colors_Line3_mesh2[2] = {
    color3( 0.0, 1.0, 0.0),  // black
    color3( 0.0, 1.0, 0.0)  // 
};
...
void colorSurface(model* g_model, vector<color3> &_colors){
    for(int n=0; n< g_model->nv; n++)
    {
        if( abs(g_model->positions[n].z) <0.0085 ){
            ...
            color_element.z = 0.0;
            _colors.push_back(color_element);
        }else if(abs(g_model->positions[n].z) <0.2){
            point3 color_element;
            color_element.x = 0.0;
            color_element.y = 1.0;
            color_element.z = 0.0;
...
void position_cero_level(model* g_model, vector<curveStruct> &model_curve){
    ...
    for (int i = 0; i < 24; )
    {
        ...
        for(int n=0; n< g_model->nv; n++)
        {
            double dist = abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine );
            //(n%100 == 0)cout<<n<<endl;
            if( dist > i*ratio && dist <(i+1)*ratio && g_model->positions[n].z >0){
                count++;
                ...
                //cout<<"positions:::::   "<< g_model->positions[n].z<<endl;
            ...
            /*if( abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine ) <0.0085 && abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine ) >=.085 && g_model->positions[n].z >0 ){
                count++;
                landa +=abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine ); 
                point3 pos;
                pos.x = g_model->positions[n].x;
            ...
            if( abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine ) <1 && g_model->positions[n].z >0 ){
                count++;
                landa +=abs( g_model->positions[n].z*cosine + g_model->positions[n].z*sine ); 
                point3 pos;
                pos.x = g_model->positions[n].x;
        ...
        if(positions_curve.size()<150) continue;
        else i++;
...
void init(){
    ...
    //line
    glGenBuffers(1, &line_buffer1_mesh2);
    glBindBuffer(GL_ARRAY_BUFFER, line_buffer1_mesh2);
    glBufferData(GL_ARRAY_BUFFER, sizeof(point3)*2 + sizeof(color3)*2,NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(point3)*2, Line1_mesh2);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(point3)*2, sizeof(color3)*2,vertex_colors_Line1_mesh2);

    glGenBuffers(1, &line_buffer2_mesh2);
    glBindBuffer(GL_ARRAY_BUFFER, line_buffer2_mesh2);
    glBufferData(GL_ARRAY_BUFFER, sizeof(point3)*2 + sizeof(color3)*2,NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(point3)*2, Line2_mesh2);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(point3)*2, sizeof(color3)*2,vertex_colors_Line2_mesh2);

    glGenBuffers(1, &line_buffer3_mesh2);
    glBindBuffer(GL_ARRAY_BUFFER, line_buffer3_mesh2);
    glBufferData(GL_ARRAY_BUFFER, sizeof(point3)*2 + sizeof(color3)*2,NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(point3)*2, Line3_mesh2);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(point3)*2, sizeof(color3)*2,vertex_colors_Line3_mesh2);
...
void display(){
    ...
    mat4 matrixModelLine= mat4(1.0f);
    matrixModelLine = Translate(0.0, 0.0, -4.0) * Scale (1.0, 1.0, 1.0) * Rotate(angle, 0.0, 2.0, 0.0);
    mat4 matrixModelMesh2= mat4(1.0f);
    matrixModelMesh2 = Translate(-centroide_model2.x +0.0,-centroide_model2.y +0.0,-centroide_model2.z -4.0) * Scale (1.0, 1.0, 1.0) * Rotate(angle, 0.0, 2.0, 0.0);
    ...
    glUniformMatrix4fv(u_tModel, 1, GL_TRUE, matrixModelMesh2); // GL_TRUE: matrix is row-major
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
glUniform1f( glGetUniformLocation(program, "flag"), 0.0 );
    glUniformMatrix4fv(u_tModel, 1, GL_TRUE, matrixModelLine); // GL_TRUE: matrix is row-major
    glUniform1f( glGetUniformLocation(program, "texture_mapped_ground"), 0.0 );
           // Wireframe floor
       glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    drawObj2(line_buffer1_mesh2, 2);  // draw the floor
    ...
    glUniform1f( glGetUniformLocation(program, "texture_mapped_ground"), 0.0 );
            // Wireframe floor
       glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    drawObj2(line_buffer2_mesh2, 2);  // draw the floor
    ...
    glUniform1f( glGetUniformLocation(program, "texture_mapped_ground"), 0.0 );
             // Wireframe floor
       glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    drawObj2(line_buffer3_mesh2, 2);  // draw the floor
