...
vec3 init_eye(7.0, 3.0, 0.0);
vec3 eye = init_eye;
vec3 _cameraDir( -7.0f, -3.0f, 0.0f );
...
void colorSurface(model* g_model, vector<color3> &_colors){
    ...
    {
        if( abs(g_model->positions[n].z-3) <0.0085 ){
            //cout<<"entro"<<endl;
...
void position_cero_level(model* g_model, vector<curveStruct> &model_curve ){
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        curveStruct c = {positions_curve,q};
...
double longitud_curve(vector< point3 > positions)
{
    ...
    cout<<"LONGITUD CURVA:  "<< distance_curve <<endl;
